 ...
 */
package org.democommercewebservices.v2.controller;
...
import org.democommercewebservices.constants.YdemocommercewebservicesConstants;
import org.democommercewebservices.formatters.WsDateFormatter;
import org.democommercewebservices.product.data.ReviewDataList;
import org.democommercewebservices.product.data.SuggestionDataList;
import org.democommercewebservices.queues.data.ProductExpressUpdateElementData;
import org.democommercewebservices.queues.data.ProductExpressUpdateElementDataList;
import org.democommercewebservices.queues.impl.ProductExpressUpdateQueue;
import org.democommercewebservices.stock.CommerceStockFacade;
import org.democommercewebservices.v2.helper.ProductsHelper;
import org.democommercewebservices.validator.PointOfServiceValidator;
...
public class ProductsController extends BaseController
{
	...
	static
	{
		PRODUCT_OPTIONS = Arrays.stream(ProductOption.values()).map(ProductOption::toString)
				.collect(Collectors.joining(YdemocommercewebservicesConstants.OPTIONS_SEPARATOR));
	...
	{
		final String[] optionsStrings = options.split(YdemocommercewebservicesConstants.OPTIONS_SEPARATOR);
	...
	public ProductReferenceListWsDTO getProductReferences(
			...
			@ApiParam(value = "Reference type according to enum ProductReferenceTypeEnum. If not specified, all types of product references will be used.") @RequestParam(required = false) final String referenceType,
			@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		final List<ProductReferenceTypeEnum> productReferenceTypeList = StringUtils.isNotEmpty(referenceType) ?
				List.of(ProductReferenceTypeEnum.valueOf(referenceType)) :
